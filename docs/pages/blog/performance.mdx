...
Ethereum blocks, transactions, traces, logs, and events are grouped into **events**. The simplified goal of any performance work is to increase the number of events processed per second.

## Performance optimizations

[we did a lot more than this]
...
As previously mentioned, Ponder maintains an in-memory database cache. What happens when the cache gets too large? At first we used a simple LRU algorithm to evict database rows. Because of the inherent randomness of onchain events, [LRU is bad].
