...
export async function buildConfigAndIndexingFunctions({
  ...
  const perNetworkIndexingBuild = new Map<string, IndexingBuild>();

  for (const network of networks) {
    perNetworkIndexingBuild.set(network.name, {
      network,
      ...
      if (filter.type === "log") {
        if (registeredLogEvents.length > 0) {
          ...
          for (const eventName of registeredLogEvents) {
            ...
            perNetworkIndexingBuild.get(network.name)!.eventCallbacks.push({
              type: "contract",
              filter,
  ...
  for (const source of flattenSources(config.blocks ?? {})) {
    ...
    if (indexingFunction) {
      perNetworkIndexingBuild.get(network.name)!.filters.push(blockFilter);
      perNetworkIndexingBuild.get(network.name)!.eventCallbacks.push({
        type: "block",
        filter: blockFilter,
        callback: indexingFunction.fn,
  ...
  // Filter out any networks that don't have any event callbacks registered.
  for (const [networkName, indexingBuild] of perNetworkIndexingBuild) {
    if (indexingBuild.eventCallbacks.length === 0) {
      logs.push({
        level: "warn",
        msg: `No sources registered for network '${networkName}'`,
      });
      perNetworkIndexingBuild.delete(networkName);
    }
  }

  return {
    indexingBuild: Array.from(perNetworkIndexingBuild.values()),
    logs,
  };
}

export async function safeBuildConfigAndIndexingFunctions(params: {
  common: Common;
  config: Config;
  indexingFunctions: IndexingFunctions;
}) {
  try {
    const result = await buildConfigAndIndexingFunctions(params);
